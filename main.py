...
while True:
    ...
    if opcao == "1":
        ...
        start_crip = time.perf_counter()
        criptografar_arquivo(arquivo)
        end_crip = time.perf_counter()
        t_crip = end_crip - start_crip
        
        print("Arquivo criptografado com sucesso!")
        logging.info(f"ARQUIVO CRIPTOGRAFADO (SIMETRICA): {arquivo}.enc | Tempo de execucao {t_crip:.6f} s")

    elif opcao == "3":
        arquivo_encriptado = input("Caminho do arquivo encriptado '.enc': ")
        
        start_decrip = time.perf_counter()
        arquivo_decriptado = decriptografar_arquivo(arquivo_encriptado)
        end_decrip = time.perf_counter()
        t_decrip = end_decrip - start_decrip
        
        print("Arquivo decriptado!")
        logging.info(f"ARQUIVO DECRIPTADO (SIMETRICA): {arquivo_decriptado} | Tempo de execucao {t_decrip:.6f} s")
        ...
        t_rsa = end_rsa - start_rsa

        print("Arquivo criptografado com a sua Chave Pública!")
        logging.info(f"ARQUIVO CRIPTOGRAFADO (ASSIMETRICA): {arquivo}.rsa | Tempo de execucao {t_rsa:.6f} s")
        ...
        end_dec_rsa = time.perf_counter()
        t_dec_rsa = end_dec_rsa - start_dec_rsa

        print("Arquivo decriptado!")
        logging.info(f"ARQUIVO DECRIPTADO (ASSIMETRICA): {arquivo_decriptado} | Tempo de execucao {t_dec_rsa:.6f} s")
